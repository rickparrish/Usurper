...
Const            {version number also in DDPLUS.PAS}
     uver      = '0.23f';
     uversion  = 'Usurper ' + uver; {prog & version}
     ucomp     = '07/08/2013';      {compile date, mm/dd/yyyy}
...
begin {* Main Program *}
  ...
  S := '';
  Write('   - Looking for ' + Global_VersionF + '...');
  ...
  begin
    ...
    begin
      ...
      if (Open_TxtFile(TReset, Txt, Global_VersionF)) then
      begin
        WriteLn('SUCCESS');
        ReadLn_From_Text(Txt, S);
        Close_Text(Txt);
        WriteLn('   - Version from file: ' + S);
        break;
      end else
      begin
        WriteLn('FAILED');
  ...
  begin
    ...
    WriteLn;
    WriteLn('   - Please run EDITOR to upgrade your version of Usurper');
    WriteLn('   - Aborting...');
    WriteLn;
    Halt;
  end;
...
{$ENDIF}
 ...
 if param_hunt('?') or
    ...
    param_hunt('/DO_PACKCHILDREN') then begin
 ...
 if param_hunt('/SCORES') then begin
  {Update Scores files}
  wrl(10,'');
  wrl(10,'');
 ...
 if param_hunt('/DO_PACKMAIL') then begin
  {Pack Mail file}
  pack_mail;
 end;

 if param_hunt('/DO_PACKRELATIONS') then begin
  {Pack Relation file}
  pack_relations;
 end;

 {Pack Children data-file}
 if param_hunt('/DO_PACKCHILDREN') then begin
  pack_children;
 end;

 {Pack Moat file}
 if param_hunt('/DO_PACKMOAT') then begin
  pack_moat;
 end;

 {Pack User file}
 if param_hunt('/DO_PACKUSERS') then begin
  pack_users(PUsers);
 end;

 if param_hunt('/DO_PACKNPCS') then begin
  {Pack NPC file}
  pack_users(PNpcs);
 end;

 if param_hunt('/DO_SENDMAIL') then begin
  {Send mail to user}
  send_a_letter(true);
 end;

 if exit_ordered=true then begin
  normal_exit;
 end;

 {does maintfile exists and has it existed too long?}
 if f_exists(global_maintfile)=true then begin
  kill_file:=false;
  ...
  if i>0 then begin
   ...
   if i>0 then begin
    ...
    if i>0 then begin
     ...
     if i>45 then kill_file:=true;
    end;
   end;
  end;

  if kill_file then begin
 ...
 {Determine Graphics}
 if (graphics=1) or (graphics=2) then global_ansi:=false
 ...
 {Date file}
 if f_exists(global_datefile)=false then begin
  d(config.textcolor,'Could not find the date file.');
  d(config.textcolor,'Creating a new one...');

  if open_txtfile(trewrite,txt,global_datefile) then begin
 ...
 if param_hunt('/HMAINT')=true then begin
  exit_ordered:=true;
  order_maint:=true;
 end;

 if (param_hunt('/FMAINT')=true) and (order_maint=false) then begin
exit_ordered:=true;
  if open_txtfile(treset,txt,global_datefile) then begin
   readln_from_text(txt,s);
   close_text(txt);
  end
  else begin
 ...
 if (Config.AutoMaint=true) and (order_maint=false) then begin
  if open_txtfile(treset,txt,global_datefile) then begin
   readln_from_text(txt,s);
   close_text(txt);
  end
 ...
 if Order_Maint then begin
  ...
  if f_exists(global_onfile) then begin
   for i:=1 to fs(FsOnline) do begin
    ...
    if (person.name<>'') and (person.shadow=false) then begin
     dimmy:=true;
     break;
    end;
   end; {for i:= .end.}
  ...
  else if f_exists(global_maintfile) then begin
 ...
 {Do we need to move the .doc files?}
 i:=1;
 first:=true;
 repeat
  case i of
  ...
  if f_exists(s) then begin
   if f_exists(global_docsdir+s) then delete_file(global_docsdir+s);
   if first=true then begin
    first:=false;
    d(config.textcolor,'Updating Usurper files... please wait!');
   end;
   move_file(s,global_docsdir)
  end;
  inc(i);
 until i>10;

 {Need to move TEXT.DAT file?}
 s:=global_textdataf;
 if f_exists(s) then begin
  if f_exists(global_datadir+s) then delete_file(global_datadir+s);
  d(config.textcolor,'Updating Usurper files... please wait!');
 ...
 if not config.fastplay then begin
 ...
 if (not global_registered) and (global_ubeta) then begin
  crlf;
  crlf;
 ...
 repeat
  ...
  case ch of
   ...
   'E':begin {enter as GOD}
        ...
        if (config.bothImmortalsAndMortals=false) and (user_search(username,false)=true) then begin
         play:=false;
         crlf;
        ...
        if (play) and (god_search(username)=true) then begin
         ...
         if global_utest then begin
          ...
          if confirm('Create a God','N')=true then begin
   ...
   'M':begin {Send message to online player}
        ...
        d(11,'Message');
        crlf;
        if confirm('Send Anonymously','N')=true then begin
   ...
   'S':begin {display Game Settings, various3.pas}
        ...
        if config.viewgamesettings then begin
         display_game_settings(uversion);
        end
        else begin
         crlf;
         crlf;
   ...
   'N':begin {News-Paper}
        ...
        d(11,'News');
        crlf;
        if global_ansi then display_file(global_nwfileans)
   ...
   'Y':begin {Yesterday news}
        ...
        crlf;
        if global_ansi then display_file(global_ynwfileans)
   ...
   'T':begin {List of Teams}
        ...
        crlf;
        if global_ansi then s:=cfg_string(72)
   ...
   'I':begin {Instructions}
        ...
        d(15,'Instructions');
        crlf;
        display_file(global_docsdir+'USURPER.DOC');
        pause;
   ...
   'P':begin {Play The Game!}
        ...
        if global_ubeta then begin
         ...
         d(13,'ALERT! This is an early test-version of '+uversion+'.');
        ...
        if (config.bothImmortalsAndMortals=false) and (player_has_a_god(username)=true) then begin
        ...
        if (config.simulnode=false) and (play=true) then begin

         {check if user is alredy online}
         if f_exists(global_onfile)=true then begin
        ...
        if play=true then begin
         ...
         if f_exists(global_maintfile) then begin
          {read maint.flg data}
          s:=''; {node}
          if open_txtfile(treset,txt,global_maintfile) then begin
         ...
         if ch='B' then begin
          ...
          if user_search(username,true) then begin
   ...
   'A':begin {Author information}
		...
		d(14,'            Win32/Linux/OS/2 Ports : Rick Parrish usurper@rickparrish.ca');
		d(14,'   Post 0.20e Fixes & Improvements : Dan dan1982code@hotmail.ca');
        crlf;
   ...
   'Q':begin {Quit}
        d(11,'Quit');
        crlf;
        if confirm('Really QUIT','Y') then begin
         normal_exit;
        end;
       end;
