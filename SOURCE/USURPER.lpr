...
uses
  Dos, DDplus, Init, Jakob, Maint, Cms, SortPl, VarGods, GodWorld, UserHunc, Gamec, Byebyec, Mail, Various, Various2, Rating, Relation, Relatio2, Post_To, Online, File_Io, Various3, News, Children, InitGods, SysUtils, VideoUtils;
...
var
  ...
  kill_file, exit_ordered, order_maint, dimmy, First, play: boolean;
...
begin {* Main Program *}
 ...
 {MAINTENANCE/SCORES/PACKMAIL/PACKMOAT/PACKUSERS/PACKNPCS/SENDMAIL
  ...
  if (param_hunt('/FMAINT') = True) and (order_maint = False) then
  begin
    exit_ordered := True;
  ...
  begin
    ...
    begin
      ...
      wrl(12, '');
    end else if f_exists(global_maintfile) then
    begin
